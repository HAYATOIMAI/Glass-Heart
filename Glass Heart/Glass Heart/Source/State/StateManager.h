...
namespace GlassHeart {
    ...
    namespace State {
        ...
        public:
             ...
             */
            StateManager(std::string_view key, std::shared_ptr<AppFrame::StateBaseRoot> state);
            /**
             * @brief ステートの登録
             * 
             * @param[in] key   最初のステートに関連付ける任意の文字列
             * @param[in] state 最初のステート
             */
            void Register(std::string_view key, std::shared_ptr<AppFrame::StateBaseRoot> state);
            /**
             * @brief ステートのプッシュバック
